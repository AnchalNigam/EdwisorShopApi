    ...
    let addingProductFunction = () => {
        return new Promise((resolve, reject) => {
            if (check.isEmpty(req.body.productId)) {
                ...
                logger.error(`Error Occured : Required Parameters are missing!`, 'Cart Controller: addProduct', 100);
...
let deleteProductCart=(req,res)=>{
    if (check.isEmpty(req.params.cartId)) {
        ...
        let apiResponse = response.generate(true, 'CartId is missing rebase-change', 403, null)
    ...
    if (check.isEmpty(req.params.userId)) {
        ...
        .exec((err, result) => {
            if (err) {
                ...
                let apiResponse = response.generate(false, "All Products of your cart Found successfully!!!!! master", 200, result);
                res.send(apiResponse);
